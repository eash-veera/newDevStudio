<?php
	...
	class ProjectService {
		...
		private function create(){
			...
			$destFolder = STORAGE_LOCATION . "/" . $appKey;
			...
			switch($pType){
				...
				case "bundle":
					...
					$client = ObjectStoreClient::WithNamespace("duoworld.duoweb.info","appbundle","123");
			...
			$client = ObjectStoreClient::WithNamespace("duoworld.duoweb.info","projectmappingtenant","123");
		...
		private function getGenericMappings($index,$key,$value){
			$client = ObjectStoreClient::WithNamespace("duoworld.duoweb.info",$index,"123");
			...
			foreach($allMappings as $mapping){
				$client = ObjectStoreClient::WithNamespace("duoworld.duoweb.info","projects","123");
				$oneProject = $client->get()->byKey($mapping["appKey"]);
				array_push($outData, $oneProject);
			}
		...
		function download($appKey,$filename){
		    $folder = STORAGE_LOCATION . "/" . $appKey;
		    $zipname = STORAGE_LOCATION . "/" . $appKey . "/" . $filename;
		...
		function appIconUpload($appKey){
			$fileName = APPICON_PATH . "/" . $appKey . ".png";
			$content = Flight::request()->getBody();
			if ($content =="N/A")
				copy(APPICON_PATH . "/default.png", $fileName);
			else
		...
		function deleteProject($appKey){
			...
			$client_ps = ObjectStoreClient::WithNamespace("duoworld.duoweb.info","projectshares","123");
		...
		function getShares($appKey){
			$client = ObjectStoreClient::WithNamespace("duoworld.duoweb.info","projectshares","123");
		...
		function addShare($appKey,$userName){
			...
			$client = ObjectStoreClient::WithNamespace("duoworld.duoweb.info","projectmappinguser","123");
			$client->store()->byKeyField("mapId")->andStore($saveObj);
		}
		...
		private function publish($appKey){
			(new Publisher($appKey,[MAIN_DOMAIN, $_SERVER["HTTP_HOST"] ]))->Publish();
		}
		...
		function __construct(){
			...
			Flight::route("GET /project/publish/@appKey", function ($appKey){ $this->publish($appKey); });
